...
function MagneticButton({ children, className, ...props }: React.ComponentProps<'button'> & { className?: string }) {
...
function MagneticLink({ children, className, ...props }: React.ComponentProps<'a'> & { className?: string }) {
...
export function Hero() {
  ...
  const sbgX = useSpring(bgX, { stiffness: 15, damping: 20 });
  const sbgY = useSpring(bgY, { stiffness: 15, damping: 20 });
  ...
  const onMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    if (!rect) return;
    bgX.set((e.clientX - rect.left) / rect.width - 0.5);
    bgY.set((e.clientY - rect.top) / rect.height - 0.5);
  }, [reduced, bgX, bgY]);
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        style={{
          background: 'radial-gradient(circle at center, rgba(108,99,255,0.09) 0%, transparent 65%)',
        }}
        animate={reduced ? {} : {
          scale: [1, 1.05, 1],
          opacity: [0.9, 1, 0.9],
      ...
      <div className="max-w-7xl mx-auto px-6 w-full py-16 grid lg:grid-cols-[55%_45%] gap-10 lg:gap-6 items-center">
        ...
        >
          {/* Role label */}
          <motion.div variants={reduced ? {} : item} className="flex items-center gap-3 mb-7">
            <motion.span
          ...
          >
            {reduced ? (
              <>
                <span className="block text-muted text-4xl sm:text-5xl lg:text-[3.5rem] mb-0.5">Hi, I&apos;m</span>
                <span className="block text-foreground text-6xl sm:text-7xl lg:text-[5.5rem]">Sakshi.</span>
              </>
            ) : (
              <>
                <WordReveal text="Hi, I'm" className="block text-muted text-4xl sm:text-5xl lg:text-[3.5rem] mb-0.5" delay={0.3} />
                <WordReveal text="Sakshi." className="block text-foreground text-6xl sm:text-7xl lg:text-[5.5rem]" delay={0.5} />
              </>
            )}
          </motion.h1>
          ...
          <motion.div variants={reduced ? {} : item} className="flex flex-wrap gap-3 mb-9">
            <MagneticButton
          ...
          {/* Social links */}
          <motion.div variants={reduced ? {} : item} className="flex items-center gap-0.5">
            {[
              ...
              >
                ...
                {i < arr.length - 1 && <span className="w-px h-4 bg-border mx-0.5" />}
              </motion.span>
            ))}
          </motion.div>
        </motion.div>
